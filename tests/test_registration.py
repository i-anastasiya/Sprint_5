...
class TestStellarBurgersRegistration:
    def tests_registration(self, registration):
        ...
        registration.find_element(*Locators.AUTH_BUTTON_REGISTRATION).click()
        # Добавляю ожидание, чтобы прогрузился следующий экран авторизации
        button = WebDriverWait(registration, 5).until(EC.visibility_of_element_located
                                                      ((Locators.AUTH_BUTTON_ACCOUNT))).text
        assert button == 'Войти'
        registration.find_element(*Locators.EMAIL).send_keys(name)
        registration.find_element(*Locators.PASSWORD).send_keys(email)
        registration.find_element(*Locators.AUTH_BUTTON_ACCOUNT).click()
